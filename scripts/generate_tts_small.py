#!/usr/bin/env python3
"""
P0: 真实 TTS 数据 - 小规模快速版（100 样本）

使用 edge-tts 生成真实人声，但样本量小以加快速度。
"""
...
SAMPLE_RATE = 16000
OUTPUT_DIR = Path("data/tts_task3_small")
MANIFEST_DIR = Path("manifests/tts_task3_small")
...
async def process_sample(i: int, split_dir: Path, split_name: str):
    ...
    try:
        ...
        # 转换为 wav
        for mp3_path, wav_path in [(input_mp3, input_wav), (output_mp3, output_wav)]:
...
async def main():
    print("=" * 60)
    print("P0: Small TTS Dataset (100 samples)")
    print("=" * 60)
