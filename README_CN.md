...
| 2026-01-02 | **睡眠机制研究** | 选择性 >> 随机 (45pp)；k=0.7 在 64+ 符号时有效 (+2pp) |
...
| **三角验证** | Audio↔IPD↔RF 9/9 | 载体无关证据 |
| **选择性 vs 随机** | 比 Dropout 好 45pp | 选择性重要，但完全保留最优 |

> wav2vec2 测试了冻结、部分微调（32%）、全参数微调三种设置——均只达到 13%。这证实了任务特化架构的必要性。

### 🧠 类生物学发现

我们对 SSM 隐状态动态进行了系统性实验：

| 实验 | 方法 | 结果 |
|------|------|------|
| **硬重置** | `h *= decay` | ❌ 性能崩溃 |
| **随机 Dropout** | 随机丢弃 50% | ❌ -48pp 置信度 |
| **选择性修剪** | 保留前 50% | ⚠️ -2.6pp (10-seed) |
| **可学习 Gate** | 模型自己决定 | ✅ **学到 keep=100%** |

**关键发现**：
1. **选择性 >> 随机性**：选择性修剪比 Dropout 好 45pp
2. **短序列需要完全保留**：可学习 Gate 选择 keep=100%
3. **长序列从修剪中获益**：64+ 符号时，k=0.7 提升 +1.5~2.2pp
4. **存在临界长度**：约 64 符号是修剪变得有益的阈值

> 状态累积对短序列有益，但对长序列成为限制因素。温和的"突触下调"（k=0.7）有助于在临界长度之后保持性能。

---

这与神经科学的**突触稳态假说 (SHY)** 相关：

| 我们的实验 | 生物学机制 | 效果 |
|-----------|-----------|------|
| 硬重置 `h *= 0.5` | 全脑抑制（昏迷） | ❌ 能力丧失 |
| 选择性 `keep=0.5` | NREM 突触缩放 | ✅ 能力提升 |
| `keep=0.3`（过度修剪） | 神经退行性疾病 | ❌ 认知崩溃 |

> 睡眠不是被动休息，而是**主动的选择性突触修剪**。连续波形模型可能受益于类生物学的状态维护机制。
...
> **关键发现**：wav2vec2 即使全参数微调也只有 13%——几乎等于随机猜测（10%）。通用语音预训练不适用于波形推理。
